...
<head>
    <title>{{ get_phrase('Payment | ClassKira') }}</title>
    <!-- all the meta tags -->
    <meta charset="utf-8" />
    ...
    <!-- all the css files -->
    <link rel="shortcut icon" href="{{ asset('assets/uploads/logo/' . (get_settings('favicon') ?: 'logo.svg')) }}">
    <!-- Bootstrap CSS -->
                        ...
                        <div class="off_payment_form">
                            ...
                            @php
                                if (session()->has('payment_for')) {
                                    $fee_details = session('fee_details');
                                    $url = route('student.offline.payment.hostel');
                                } else {
                                    $url = route('student.offline_payment', ['id' => $fee_details['id']]);
                                }
                            @endphp

                            <form action="{{ $url }}" class="offline-form form" method="post" enctype="multipart/form-data">
                                @csrf
