...
<div class="tab-pane fade " id="v-pills-razorpay" role="tabpanel" aria-labelledby="v-pills-razorpay-tab" tabindex="0" >
    ...
    <form action="{{ route('razorpay.payment.store',['success_url'=> 'student.student_fee_success_payment_student','cancle_url' => 'student.student_fee_fail_payment_student','payment_method'=>'razorpay']) }}" method="post" class="paypal-form form">
        @csrf
        ...
        <script src="https://checkout.razorpay.com/v1/checkout.js" data-key="<?= $razorpay_key?>"
            data-amount={{ $fee_details['total_amount']*100 }}
            data-buttontext="Pay by Razorpay"
            data-name={{ $fee_details['title'] }}
             data-description={{ $user_info['name'] }}
             data-image="{{ asset('assets/uploads/logo/logo.svg') }}"
             data-prefill.name={{ $user_info['name'] }}
             data-prefill.email={{ $user_info['email'] }}
             data-theme.color="<?= $razorpay_keys->theme_color?>">
            </script>
