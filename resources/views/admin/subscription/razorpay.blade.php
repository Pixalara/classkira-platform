...
<div class="tab-pane fade " id="v-pills-razorpay" role="tabpanel" aria-labelledby="v-pills-razorpay-tab" tabindex="0">
    ...
    <form action="{{ route('superadmin.razorpay.subscription',['success_url'=> 'admin_subscription_fee_success_payment','cancle_url' => 'admin_subscription_fee_fail_payment','payment_method'=>'razorpay']) }}" method="post" class="paypal-form form">
        ...
        <script src="https://checkout.razorpay.com/v1/checkout.js" data-key="<?= $razorpay_key?>" data-amount={{ $selected_package ['price']*100 }} data-buttontext="Pay by razorpay" data-name={{ $selected_package ['name'] }} data-description={{ $user_info['name'] }} data-image="{{ asset('assets/uploads/logo/logo.svg') }}" data-prefill.name={{ $user_info['name'] }} data-prefill.email={{ $user_info['email'] }} data-theme.color="<?= $razorpay_keys->theme_color?>">
        </script>
